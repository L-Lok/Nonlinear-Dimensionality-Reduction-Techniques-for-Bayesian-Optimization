...
def bo_warmup_sdr(
    ...
    sdr.initialize(original_bounds=original_bounds)
    ...
    for j in range(n_trials):
        ...
        # print("i:", j, "new", new_point_x, "fvalue", new_point_y)
        ...
        # shrink bounds
        if j % 5 == 0:
            current_bounds = sdr.transform(
                train_x=train_x, train_y=train_y)
            logging.info("SDR Current bounds: %s", current_bounds)
        ...
        j += 1
        # print("acq_val_trace", acq_val_trace)
        # print("best_value_f_trace", best_value_f_trace)
...
def bo_warmup_sdr_pipeline(
    ...
    for i in range(n_repeats):
        ...
        best_value, acq_val_trace, best_value_f_trace, train_x_trace, train_y_trace = bo_warmup_sdr(
            ...
            n_trials=n_trails,
            sdr_min_window=sdr_min_window,
            train_y_var=train_y_var
        )

        print(best_value, acq_val_trace, best_value_f_trace)
