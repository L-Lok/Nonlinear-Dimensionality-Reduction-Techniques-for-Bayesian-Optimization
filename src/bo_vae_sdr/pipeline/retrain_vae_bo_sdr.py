...
def bayesian_optimization_new_points_loop(
    ...
    for k in range(frequency):
        ...
        with torch.no_grad():
            xhat = vae_model.decoder(zhat).detach()
        print("new_xhat", xhat)
...
def retrain_vae_bo_sdr_base_pipeline(
    ...
    train_loader, test_loader = DatasetPrepare(
        hparams={
            ...
            "target_dim": 1,
            "data_split": 0.8, # do it as you want
            # "data_split": 0.9, # do it as you want --- IGN
    ...
    sdr = SequentialDomainReductionTransformer(minimum_window=sdr_min_window)

    for vae_loop_i in range(int(n_trails/n_query)):
        latent_bounds = torch.tensor([
            [-5., 5.]] * encoder_layers[-1]).to(device)
        sdr.initialize(original_bounds=latent_bounds.to(torch.float64))

        opt_params = {"lr": 1e-3}

        current_trained_vae_model, results = training_vae(
            epochs=1 + 1,
            model=current_trained_vae_model,
        ...
        merged_train_dataset_tensor_x = torch.cat(
            (
                train_loader.dataset.tensors[0].detach().to(device),
        ...
        merged_train_dataset_tensor_y = torch.cat(
            (
                train_loader.dataset.tensors[1].detach().to(device),
...
def retrain_vae_bo_sdr_pipeline(
    test_func: BaseTestFunction,
    encoder_layers: List[int] = [5, 3, 2],
    n_initial_points: int = 50,
    n_trails: int = 350,
    ...
    for i in range(n_repeats):
        results.append(
            retrain_vae_bo_sdr_base_pipeline(
                test_func=test_func,
