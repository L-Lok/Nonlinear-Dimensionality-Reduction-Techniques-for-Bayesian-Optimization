...
def retrain_vae_dml_bo_base_pipeline(
    ...
    for vae_loop_i in range(int(n_trails/n_query)):
        ...
        current_trained_vae_model.beta_metric_loss = 1
        current_trained_vae_model.metric_loss = {
            "type": "triplet",
            "eta": 0.05,
            "margin": None,
            "soft": True,
...
def retrain_vae_dml_bo_pipeline(
    test_func: BaseTestFunction,
    encoder_layers: List[int] = [5, 3, 2],
    n_initial_points: int = 50,
    n_trails: int = 350,
    ...
    for i in range(n_repeats):
        results.append(
            retrain_vae_dml_bo_base_pipeline(
                test_func=test_func,
